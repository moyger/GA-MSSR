...
class KhushiSignalEngine:
    ...
    def update_model(self, ga_weights: list[float], rule_params: list) -> None:
        """Hot-swap GA weights and rule parameters, preserving the OHLC buffer."""
        self._weights = np.array(ga_weights, dtype=np.float64)
        self._rule_params = []
        for p in rule_params:
            if isinstance(p, (list, tuple)) and len(p) == 1:
...
try:
    ...
    class KhushiStrategy(Strategy):
        ...
        def update_from_model(self, ga_weights: list[float], rule_params: list) -> None:
            """Hot-swap model weights without losing buffer state."""
            self._engine.update_model(ga_weights, rule_params)
            self.log.info(f"Model hot-swapped: {len(ga_weights)} weights updated")
