...
def _set_datetime_index(
    ...
    if col and col in df.columns:
        # Detect DD.MM.YYYY or DD/MM/YYYY format (e.g. forex reference data)
        sample = str(df[col].iloc[0])
        dayfirst = len(sample) >= 3 and sample[:2].isdigit() and sample[2] in "./"
        df[col] = pd.to_datetime(df[col], utc=True, dayfirst=dayfirst)
        df = df.set_index(col)
